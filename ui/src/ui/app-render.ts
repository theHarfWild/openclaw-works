...
import './components/latex-viewer.js'
...
export function renderApp(state: AppViewState) {
  ...
  const renderConfigTabForActiveTab = () => {
    switch (state.tab) {
      case "paper": {
        // Auto-load paper task list on first render
        if (!state.paperTaskList || state.paperTaskList.length === 0) {
          void (async () => {
            if (!state.client || !state.connected) return;
            try {
              const result = await state.client.request("paper.workspace.list", {});
              ...
              state.paperTaskList = tasks;
              // Auto-select: pick the first task that has a PDF, or last modified
              if (tasks.length === 1) {
                state.currentPaperTaskId = tasks[0].taskId;
              } else if (tasks.length > 0 && !state.currentPaperTaskId) {
                // Try to pick one with PDF first
                const withPdf = tasks.find((t) => t.hasPdf);
                if (withPdf) {
                  state.currentPaperTaskId = withPdf.taskId;
                }
              }
              requestHostUpdate?.();
            } catch {
        ...
        return html`
          <section class="content-header">
            <div class="page-title">${t("tabs.paper")}</div>
            <div class="page-meta">
              <input
                type="text"
                .value=${state.currentPaperTaskId || ""}
                placeholder="${t("paper.enterTaskId")}"
                list="paper-task-list"
                @change=${(e: Event) => {
                  state.currentPaperTaskId = (e.target as HTMLInputElement).value.trim();
                  requestHostUpdate?.();
                }}
              />
              <datalist id="paper-task-list">
                ${(state.paperTaskList ?? []).map(
                  (t) => html`<option value=${t.taskId}>${t.taskId}${t.hasPdf ? " ✓" : ""}</option>`,
                )}
              </datalist>
              <button
                class="btn btn--sm"
                @click=${async () => {
                  if (!state.client || !state.connected) return;
                  const id =
                    state.currentPaperTaskId?.trim() ||
                    "paper_" + Date.now().toString(36);
                  try {
                    await state.client.request("paper.workspace.init", {
                      taskId: id,
                    });
                    state.currentPaperTaskId = id;
                    // Refresh task list
                    const result = await state.client.request("paper.workspace.list", {});
                    state.paperTaskList = (result?.tasks ?? []) as Array<{
                      taskId: string;
                      taskDir: string;
                      hasMainTex: boolean;
                      hasPdf: boolean;
                    ...
                    state.paperCompileError = String(err);
                    requestHostUpdate?.();
                  }
                }}
              >
              ...
              ${state.currentPaperTaskId
                ? html`<span class="pill" style="font-size:0.8rem;margin-left:0.5rem">
                    Task: ${state.currentPaperTaskId}
                  </span>`
                : ""}
              ${state.paperCompileError
                ? html`<span class="pill danger">${state.paperCompileError}</span>`
                : ""}
            </div>
          </section>
          <paper-viewer
            .client=${state.client}
            .taskId=${state.currentPaperTaskId || null}
            .autoReload=${true}
          ></paper-viewer>
        `;
      }
